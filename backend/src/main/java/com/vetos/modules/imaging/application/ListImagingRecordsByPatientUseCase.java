...
public class ListImagingRecordsByPatientUseCase {
    ...
    public List<ImagingRecordSummary> execute(UUID patientId) {
        var patient = patientLookupPort.findSummaryById(patientId);
        var owner = ownerLookupPort.findSummaryById(patient.ownerId());

        return imagingRecordRepository.findByPatientId(patientId).stream()
            .map(record -> toSummary(record, patient.name(), owner))
            .sorted(Comparator.comparing(ImagingRecordSummary::requestedAt).reversed())
            .toList();
    }

    private ImagingRecordSummary toSummary(ImagingRecord record, String patientName, OwnerSummary owner) {
